...
func BenchmarkGetOrCreateAuthor_Existing(b *testing.B) {
	...
	authorIDs, err := getOrCreateAuthorHelper(b, db, []book.Author{author})
	if err != nil {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err := getOrCreateAuthorHelper(b, db, []book.Author{author})
		if err != nil {
...
func BenchmarkGetOrCreateAuthor_Mixed(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		if i%2 == 0 {
			_, err := getOrCreateAuthorHelper(b, db, []book.Author{existingAuthors[i%3]})
			if err != nil {
				b.Fatalf("getOrCreateAuthor failed: %v", err)
			}
		} else {
			...
			_, err := getOrCreateAuthorHelper(b, db, []book.Author{newAuthor})
			if err != nil {
				b.Fatalf("getOrCreateAuthor failed: %v", err)
			}
		}
